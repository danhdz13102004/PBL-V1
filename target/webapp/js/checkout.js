    ...
    <div class="checkout-history__info-wrap">
      ...
      <div class="checkout-history__address">
        <div class="home-address">
          ${optionData.address}, ${optionData.ward}, ${optionData.district}, ${optionData.province}
        </div>
      </div>
    </div>
...
btnSubmitInfo.addEventListener(`click`, async () => {
  var url = currentURL + "/api/user/addNewAddress";
  var tenNguoiNhan = document.querySelector('#checkout-info__fullname').value;
  var soDienThoai = document.querySelector('#checkout-info__phone-number').value;
  var province = document.querySelector('.inp-province-add').value;
  var district = document.querySelector('.inp-district-add').value;
  var ward = document.querySelector('.inp-ward-add').value;
  var road = document.querySelector('#checkout-info__address').value;

  url  += `?tenNguoiNhan=${encodeURIComponent(tenNguoiNhan)}&soDienThoai=${encodeURIComponent(soDienThoai)}&province=${encodeURIComponent(province)}&district=${encodeURIComponent(district)}&ward=${encodeURIComponent(ward)}&road=${encodeURIComponent(road)}`;
...
const getCurrenData1 = (optionParent) => {
  const fullname = optionParent.querySelector(`.checkout-history__fullname`);
  const phoneNumber = optionParent.querySelector(
    `.checkout-history__phone-number`
  );
...
checkoutActionEditButtons.forEach((checkoutActionEditButton) => {
  checkoutActionEditButton.addEventListener(`click`, async () => {
    ...
    var id = checkoutActionEditButton.querySelector(".inp-id").value;
    var element = document.querySelector('.inp-id-update');
    element.value = id;
    // document.querySelector('.inp-id-update').value = id;
    targetElement = optionParent;
    getCurrenData1(optionParent);
    await loadToUpdateInfo(currentData);
    modalUpdateInfo.style.display = `flex`;
    modalSwitchInfo.style.display = `none`;
...
btnUpdateInfo.addEventListener(`click`, async () => {
  if (isValidForm(allInputs, btnUpdateInfo)) {
    ...
    const province = document.querySelector('.update-province');
    var selectedOption = province.options[province.selectedIndex];
    var provinceText = selectedOption.text;

    const district = document.querySelector('.update-district');
    var selectedOption1 = district.options[district.selectedIndex];
    var districtText = selectedOption1.text;
    
    const ward = document.querySelector('.update-ward');
    var selectedOption2 = ward.options[ward.selectedIndex];
    var wardText = selectedOption2.text;

    var roadText = document.querySelector('.road-update').value;
    console.log("text road " + roadText );
    fullname.textContent = newUpdateData.fullname;
    phoneNumber.textContent = newUpdateData.phoneNumber;
    address.textContent = roadText + ", " + wardText + ", " + districtText + ", " + provinceText;

    var id = document.querySelector('.inp-id-update').value;
    var newaddress = roadText + ", " + wardText + ", " + districtText + ", " + provinceText;
    var url = currentURL + `/api/user/updateAddress?id=${encodeURIComponent(id)}&name=${encodeURIComponent(newUpdateData.fullname)}&soDienThoai=${encodeURIComponent(newUpdateData.phoneNumber)}&address=${newaddress}`;
    console.log(url);
    fetch(url);
